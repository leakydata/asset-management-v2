"""One machine, a few questions: the CCAT decision key, animated.
...
def box(text: str, color: str, width: float = 4.4, size: int = 28) -> VGroup:
    ...
    rect = RoundedRectangle(
        corner_radius=0.18,
        width=width,
        height=max(1.0, label.height + 0.5),
        color=color,
        stroke_width=3,
    )
...
def vedge(a: VGroup, b: VGroup, text: str, color: str, side=LEFT) -> VGroup:
    """Vertical-ish edge: from a's bottom to b's top."""
    arrow = Arrow(
...
def hedge(a: VGroup, b: VGroup, text: str, color: str) -> VGroup:
    """Horizontal edge: from a's right to b's left, label above."""
    arrow = Arrow(
...
class DecisionKey(Scene):
    ...
    def construct(self) -> None:
        ...
        self.play(Write(title), FadeIn(sub, shift=UP * 0.3))
        self.wait(1.2)
        ...
        self.play(FadeIn(q1, shift=DOWN * 0.3))
        self.play(Create(e1[0]), FadeIn(e1[1]), FadeIn(q2, shift=DOWN * 0.3))
        self.play(
        ...
        self.play(FadeIn(a, shift=RIGHT * 0.4), FadeIn(a_tag))
        self.wait(0.4)
        self.travel(a, q1)
        ...
        self.play(Create(glowa), FadeIn(stamp_a), Flash(t_add.get_center(), color=GREEN, line_length=0.35))
        self.wait(0.8)

        # ---------- machine B: the reviewed transfer ----------
        ...
        self.play(FadeIn(b, shift=LEFT * 0.4), FadeIn(b_tag))
        self.wait(0.4)
        self.travel(b, q1)
        ...
        self.play(Create(glowb), FadeIn(stamp_b), Flash(t_transfer.get_center(), color=RED, line_length=0.35))
        self.wait(1.0)
        ...
        self.play(Write(closer1))
        self.play(FadeIn(closer2, shift=UP * 0.3))
        self.wait(2.0)
