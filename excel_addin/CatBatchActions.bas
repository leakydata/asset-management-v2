...
' writes a per-row outcome back to a "Result" column. No credentials live in
' this file - the proxy URL and function key are per-user registry settings.
'
...
Private Function TargetSheet() As Worksheet
    ...
    On Error Resume Next
    Set TargetSheet = wb.Worksheets(INPUT_SHEET)
    On Error GoTo 0
...
Public Sub CatSetupBatchAddUpdateSheet()
    ...
    On Error Resume Next
    Set ws = wb.Worksheets(INPUT_SHEET)
    On Error GoTo 0
    ...
    btn.OnAction = "'" & ThisWorkbook.Name & "'!CatBatchAddUpdate"
