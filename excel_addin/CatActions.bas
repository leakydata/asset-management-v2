...
' CatSetupActionsSheet once per workbook to build an "Actions" sheet with
' input cells and buttons. No credentials live anywhere in this file - the
' proxy URL and function key are per-user registry settings (Settings button).
'
...
Private Function NV(ByVal nm As String) As String
    Dim wb As Workbook: Set wb = TargetBook()   ' raises if nothing is open
    On Error Resume Next
    NV = Trim$(CStr(wb.Names(nm).RefersToRange.Value))
End Function

Private Sub SetResult(ByVal msg As String)
    Dim r As Range
    On Error Resume Next
    Set r = ActiveWorkbook.Names("act_Result").RefersToRange
    On Error GoTo 0
...
Public Sub CatSetupActionsSheet()
    ...
    On Error Resume Next
    Set ws = wb.Worksheets(ACTIONS_SHEET)
    On Error GoTo 0
    ...
    ws.Range("A2").Value = "Fill in fields, then click a button, or use the Cat Assets tab on the ribbon. Set your proxy URL and function key once via Cat Assets > Settings."
...
Private Sub AddList(ByVal ws As Worksheet, ByVal nm As String, ByVal listCsv As String)
    On Error Resume Next
    With ws.Parent.Names(nm).RefersToRange.Validation
        .Delete
