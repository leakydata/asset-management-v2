...
' THE ADD-IN NEVER HOLDS A CLIENT SECRET, and in this build it holds no
' function key either - each user's proxy URL and key live in their own
' registry hive (see CatRibbon.CatSettings). Share the .xlam freely.
'
...
' THE add-in rule. ThisWorkbook is the hidden .xlam, so building a sheet or
' reading a defined name from it would put the user's data somewhere they can
' never see. Everything user-facing goes to the workbook they are working in.
Public Function TargetBook() As Workbook
    Set TargetBook = ActiveWorkbook
    If TargetBook Is Nothing Then
        Err.Raise vbObjectError + 20, , _
            "Open or create a workbook first - there is nowhere to put the results."
    End If
    If TargetBook Is ThisWorkbook Then
        Err.Raise vbObjectError + 21, , _
            "The add-in cannot target itself. Switch to your workbook and try again."
    End If
End Function
